...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    ...
    if (!isSpeechRecognitionAPIAvaliable) {
      alert('Mlk teu computador é uma pedra, teu browser não aceita nem gravação de voz, para de usar isso daí e vai usar um browser de verdade!!!!!')
        ...
        <Dialog.Content className='fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none'>
          ...
          <form className='flex-1 flex flex-col'>
            ...
            {isRecording ? (<button
              ...
              <div className='size-3 rounded-full bg-red-600 animate-pulse' />Gravando! (clique p/ interromper)
            </button>) : (<button
              type='button'
